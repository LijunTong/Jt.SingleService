...
namespace Jt.SingleService.Service.UserSvc
{
    public class CodeTempSvc : BaseSvc<CodeTemp>, ICodeTempSvc
    {
        ...
        public async Task<List<CodeTemp>> GetListByUserIdAsync(int userId)
        {
            return await _repository.GetListAsync(x => x.UserId == userId);
        }

        public async Task<List<CodeTemp>> GetPageListByUserIdAsync(PagerReq pagerEntity, int userId)
        {
            return await _repository.GetListAsync(x => x.UserId == userId, pagerEntity);
        }


        public async Task<List<CodeTemp>> GetCodeTempsBySchemeAsync(int schemeId)
        {
